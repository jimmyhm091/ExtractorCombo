...
# --------------------------------------------------------------------
# MODIFICACIONES:
# - Se agregaron enlaces de WhatsApp en los hits
# - Más emojis en el formato de los hits
# - Frase exclusiva "Solo para guapos y sexys"
# --------------------------------------------------------------------
...
VERSION = '1.0'
# ELIMINADO: PREDEFINED_COMBO_ONLINE_URL ya no se usa
...
class FawxScanner:
    ...
    def setup_directories(self):
        ...
        # MODIFICADO: combo_dir ahora apunta a /sdcard/combo
        self.combo_dir = "/sdcard/combo"
        self.proxy_dir = os.path.join(base_dir, "Hits/✮ 𝐅𝐀𝐖𝐗 𝐌𝐔𝐋𝐓𝐈 𝐗𝐓𝐑𝐄𝐀𝐌 ✮", "PROXY")
        
        # Crear directorios necesarios
        for directory in [self.hits_dir, self.proxy_dir]:
            os.makedirs(directory, exist_ok=True)
        
        # Verificar que existe el directorio de combos
        if not os.path.exists(self.combo_dir):
            print(f"{Colors.BRIGHT_RED}¡ADVERTENCIA! El directorio {self.combo_dir} no existe.{Colors.RESET}")
            print(f"{Colors.BRIGHT_YELLOW}Creando directorio...{Colors.RESET}")
            try:
                os.makedirs(self.combo_dir, exist_ok=True)
            except Exception as e:
                print(f"{Colors.BRIGHT_RED}Error al crear directorio: {e}{Colors.RESET}")
    ...
    def save_hit(self, server, username, password, data):
        ...
        # ----- MODIFICACIÓN: Se agregan más emojis, frases y enlaces de WhatsApp -----
        detailed_content = f"""
┏•━•━•━•━•━•━•━•━•━•━•━•━•━•┓
   𝐅𝐀𝐖𝐗 𝐌𝐔𝐋𝐓𝐈 𝐗𝐓𝐑𝐄𝐀𝐌   
┗•━•━•━•━•━•━•━•━•━•━•━•━•━•┛
...
┗━━━━━━ ✮[ 🤴 ]✮ ━━━━━━┛
    ...
    def load_combo_list(self):
        # MODIFICADO: Ahora solo carga combos desde /sdcard/combo
        print(f"\n{Colors.BRIGHT_YELLOW}Archivos de Combo Disponibles en /sdcard/combo:{Colors.RESET}")
        
        # Verificar si existe el directorio
        if not os.path.exists(self.combo_dir):
            print(f"{Colors.BRIGHT_RED}¡Error! El directorio {self.combo_dir} no existe.{Colors.RESET}")
            print(f"{Colors.BRIGHT_YELLOW}Por favor, crea la carpeta 'combo' en tu SD card y coloca tus archivos .txt allí.{Colors.RESET}")
            sys.exit(1)
        ...
        try:
            ...
            print(f"{Colors.BRIGHT_RED}Error al leer el directorio: {e}{Colors.RESET}")
            sys.exit(1)
        
        if not combo_files:
            print(f"{Colors.BRIGHT_RED}¡No se encontraron archivos .txt en {self.combo_dir}!{Colors.RESET}")
            print(f"{Colors.BRIGHT_YELLOW}Por favor, coloca tus archivos de combo (formato usuario:contraseña) en esa carpeta.{Colors.RESET}")
            sys.exit(1)
        ...
        for i, file in enumerate(combo_files, 1):
            ...
            try:
                file_path = os.path.join(self.combo_dir, file)
                size = os.path.getsize(file_path)
                size_str = f"{size} bytes" if size < 1024 else f"{size/1024:.1f} KB" if size < 1024*1024 else f"{size/(1024*1024):.1f} MB"
                print(f"{Colors.ORANGE}[{i}] {Colors.YELLOW}{file} {Colors.GRAY}({size_str}){Colors.RESET}")
            except:
                print(f"{Colors.ORANGE}[{i}] {Colors.YELLOW}{file}{Colors.RESET}")
        
        # Solicitar selección
        while True:
            choice = input(f"\n{Colors.RED}Seleccione el combo (1-{len(combo_files)}): {Colors.RESET}").strip()
            
            try:
                choice_idx = int(choice) - 1
                if 0 <= choice_idx < len(combo_files):
                    selected_file = combo_files[choice_idx]
                    path = os.path.join(self.combo_dir, selected_file)
                    total_loaded = self.load_combo_from_file(path)
                    
                    if total_loaded == 0:
                        print(f"{Colors.BRIGHT_RED}¡El archivo seleccionado no contiene cuentas válidas (formato usuario:contraseña)!{Colors.RESET}")
                        continue
                    
                    print(f"{Colors.BRIGHT_GREEN}Cargadas {total_loaded} cuentas desde {selected_file}{Colors.RESET}")
                    time.sleep(1)
                    break
                else:
                    print(f"{Colors.BRIGHT_RED}¡Selección fuera de rango! Elige entre 1 y {len(combo_files)}.{Colors.RESET}")
            except ValueError:
                print(f"{Colors.BRIGHT_RED}¡Entrada inválida! Por favor ingresa un número.{Colors.RESET}")
